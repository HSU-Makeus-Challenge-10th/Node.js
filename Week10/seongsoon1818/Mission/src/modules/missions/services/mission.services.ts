import { responseFromStoreMission, CreateStoreMissionData, responseFromUserMission, StartMissionChallengeData,
  StoreMissionListData, responseFromStoreMissionList, OngoingMissionListData, responseFromOngoingMissionList,
...
import { addMission, getMissionById, findUserMissionByUserIdAndMissionId, getUserMissionById, insertUserMission,
  getMissionsByStoreId, getOngoingMissionsByUserId, completeUserMission} from "../repositories/mission.repositories.js";
...
export const getStoreMissionList = async (data: StoreMissionListData) => {
  if (!Number.isInteger(data.store_id) || data.store_id <= 0) {
    throw new Error("잘못된 가게 id입니다.");
  }

  if (!Number.isInteger(data.page) || data.page <= 0) {
    throw new Error("page는 1 이상의 정수여야 합니다.");
  }
  ...
  if (data.limit > 50) {
    throw new Error("limit은 최대 50까지 가능합니다.");
  }
  ...
  const { total, missions } = await getMissionsByStoreId({
    storeId: data.store_id,
    page: data.page,
    limit: data.limit,
  });

  return responseFromStoreMissionList(store, missions, {
    page: data.page,
    limit: data.limit,
    total,
  });
...
export const getOngoingMissionList = async (
  ...
  const { total, userMissions } = await getOngoingMissionsByUserId({
    userId: data.user_id,
    page: data.page,
    limit: data.limit,
  });

  return responseFromOngoingMissionList(userMissions, {
    page: data.page,
    limit: data.limit,
    total,
  });
...
export const completeMissionChallenge = async (
  data: CompleteMissionChallengeData
) => {
  if (!Number.isInteger(data.user_id) || data.user_id <= 0) {
    throw new Error("잘못된 사용자 id입니다.");
  }
  ...
  const userMission = await findUserMissionByUserIdAndMissionId(
    data.user_id,
    data.mission_id
  );
  ...
  if (userMission.status === "C") {
    throw new Error("이미 완료된 미션입니다.");
  }

  if (userMission.status !== "G") {
    throw new Error("진행 중인 미션만 완료할 수 있습니다.");
  }

  const completedUserMissionId = await completeUserMission(userMission.id);

  const completedUserMission = await getUserMissionById(completedUserMissionId);
  if (!completedUserMission) {
    throw new Error("미션 완료 처리에 실패했습니다.");
  }

  return responseFromUserMission(completedUserMission);
};