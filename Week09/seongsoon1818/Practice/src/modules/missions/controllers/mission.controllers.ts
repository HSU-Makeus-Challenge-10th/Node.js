...
import { Body, Controller, Get, Patch, Path, Post, Query, Route, SuccessResponse, Tags, Security, Request as TsoaRequest } from "tsoa";
...
export class MissionController extends Controller {
  ...
  public async handleStartMissionChallenge(
    ...
    const loginUser = (req as any).user as LoginUser;

    const userMission = await startMissionChallenge(
      bodyToStartMissionChallenge(String(missionId), loginUser.id)
    );
  ...
  public async handleGetOngoingMissions(
    ...
    const result = await getOngoingMissionList(
      queryToOngoingMissionList(loginUser.id, {
        page : page ?? 1,
        limit : limit ?? 5,
      })
    );
