...
import { Body, Controller, Get, Path, Post, Query, Route, Security, SuccessResponse, Tags, Request as TsoaRequest } from "tsoa";
...
export class StoreContoller extends Controller {
  ...
  public async handleCreateStoreReview(
    ...
    const review = await createStoreReview(
      bodyToCreateStoreReview(String(storeId), loginUser.id, body)
    );
