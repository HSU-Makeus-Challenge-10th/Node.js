import { insertStore, findStoreById, insertReview, getAllStoreReviews } from "../repositories/store.repository";
import { StoreCreateRequest, ReviewCreateRequest, responseFromReviews } from "../dtos/store.dto";
...
export const addReview = async (storeId: number, data: ReviewCreateRequest) => {
  ...
  const reviewId = await insertReview(data.user_id, data.rating, data.content);
  return { reviewId };
};
